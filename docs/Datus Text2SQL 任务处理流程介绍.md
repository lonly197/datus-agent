...
基于最新的系统优化与实战验证（2026-01-26），Text2SQL 任务处理流程已升级为**具备高度自愈能力的智能工作流**。系统采用证据驱动的生成架构，引入 **Preflight 预检编排**、**意图澄清节点**、**专用 SQL 验证节点**、**数仓开发版 SQL 报告**、**多层级 Schema 发现**、**Schema 充分性验证**与**动态反思纠错**机制，确保在生成 SQL 之前先完成意图分析、Schema 探查和验证，然后进行 SQL 生成和验证，若不合适则通过 Reflect 进行反思，继续尝试，直到找不到满足的表才报错终止。
...
**验证失败细分**：当验证未通过时，必须区分 SQL 验证失败 与 结果验证失败，并在报告中显式标注：

```markdown
**执行状态**: ⚠️ SQL执行完成，但验证未通过
...
- `v2.15 (2026-01-31)`：失败报告统一输出；SQL 重试默认值；失败阶段映射  
- `v2.14 (2026-01-30)`：文档一致性修复；移除冗余内容；统一四阶段 Schema 发现  
- `v2.13 (2026-01-29)`：节点处理流程图；完整流程图可视化；Reflect 恢复策略映射  
- `v2.12 (2026-01-28)`：外部知识增强；新增配置参数；术语覆盖提升；Schema 自动检测与重建  
- `v2.11 (2026-01-26)`：SQL 审查说明迁移至 SQL Review 文档；工作流对比；交叉引用  
- `v2.10 (2026-01-23)`：Preflight Orchestrator；硬失败机制；多数据库支持；LLM Schema 匹配；渐进式匹配  
- `v2.8 (2026-01-16)`：意图澄清节点；SQL 验证节点；工作流终止机制；相似度阈值过滤；Context Search 优化；批量 Schema 查询；元数据自动修复；多层缓存机制
