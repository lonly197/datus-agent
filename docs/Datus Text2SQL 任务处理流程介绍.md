...
基于最新的系统优化与实战验证（2026-01-17），Text2SQL 任务处理流程已升级为**具备高度自愈能力的智能工作流**。系统采用了证据驱动的生成架构，引入了**意图澄清节点**、**专用 SQL 验证节点**、**数仓开发版 SQL 报告**、**多层级 Schema 发现**、**Schema 充分性验证**与**动态反思纠错**机制，确保在生成 SQL 之前先完成意图分析、Schema 探查和验证，然后进行 SQL 生成和验证，若不合适则通过 Reflect 进行反思，继续尝试，直到找不到满足的表才报错终止。

**v2.9 新增特性**（2026-01-17）：

- ✅ **数仓开发版 SQL 报告**：面向数据仓库开发者的 6 部分综合报告
- ✅ **带注释的 SQL**：自动为 SQL 添加业务逻辑注释（CTE 说明、字段注释、条件解释）
- ✅ **表和字段详情**：从 DDL 提取表/字段注释，展示关联关系
- ✅ **执行验证报告**：明确区分"0 行数据"与"SQL 错误"
- ✅ **优化建议生成**：基于 SQL 结构分析提供性能优化建议
- ✅ **13 个辅助方法**：DDL 解析、SQL 结构分析、字段用途推断等
...
- **三阶段 Schema 发现**：语义/关键词/LLM → Context 深度搜索 → DDL 回填
...
**触发条件**：SchemaStorage 为空或缺少表定义

**执行流程**:

1. 检测缺失的表定义（通过 `get_table_schemas()` 批量查询）
2. 调用 `_ddl_fallback_and_retry()` 从数据库获取 DDL
3. 使用 `update_table_schema()` 更新或插入表定义
4. 重新执行 Schema 搜索
...
- 验证失败时使用 `ActionStatus.SOFT_FAILED`
- 设置 `allow_reflection=True` 确保继续到 reflect 节点
...
- **13 个辅助方法**：在 `datus/api/event_converter.py` 中实现
...
6. ✅ **13 个辅助方法**：DDL 解析、SQL 结构分析、字段用途推断等
...
- 🎯 **提高精度**：意图澄清 + 相似度阈值过滤
- 🔄 **增强召回**：Context Search 更频繁触发 + DDL 回填
