...
> **相关文档**: [SQL 审查任务处理流程](Datus%20SQL%20Review%20任务处理流程介绍.md)
...
| 预检工具 | 4 个必需工具 | 7 个强制工具 |

**详细文档**: SQL 审查任务的完整处理流程请参阅 [SQL 审查任务处理流程介绍](Datus%20SQL%20Review%20任务处理流程介绍.md)

## 变更历史

### v2.16 (2026-02-03) - 文档结构优化

- **文档拆分**：SQL 审查相关内容迁移至独立文档
- **变更历史迁移**：版本历史从文档开头移至底部
- **精简对比表**：保留核心差异对比，移除冗余细节
...
- **外部知识增强**：在 Schema Discovery 阶段支持从 `ext_knowledge` 表检索指标/指标定义
- **新增配置参数**：`external_knowledge_enabled` 和 `external_knowledge_top_n`
...
- **相似度阈值过滤**：语义搜索增加 0.5 相似度阈值
- **Context Search 优化**：触发阈值从 3 提升到 10
- **批量 Schema 查询**：新增 `get_table_schemas()` 方法
- **元数据自动修复**：`update_table_schema()` 方法支持 DDL 回填
- **多层缓存机制**：L1/LLM/CLI 三层缓存

### 核心价值（历史保留）

- 🎯 **提高精度**：意图澄清 + 相似度阈值过滤
- 🔄 **增强召回**：Context Search + Fallback 全表扫描
