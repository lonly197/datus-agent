...
class SQLReviewPreflightOrchestrator:
    ...
    def __init__(
        ...
        # Tool classification
        self.legacy_tools = {
            "describe_table", "search_external_knowledge", "read_query", "get_table_ddl"
        }
        self.enhanced_tools_set = {
            "analyze_query_plan", "check_table_conflicts", "validate_partitioning"
        }
    ...
    async def run_preflight_tools(
        ...
        for tool_name in required_tools:
            ...
            try:
                ...
                yield self._create_tool_action(
                    tool_name, result, execution_time, tool_call_id
                )
                ...
                # Send error event
                await self._send_tool_error_event(
                    tool_name, str(e), execution_id, tool_call_id
                )
                ...
                yield self._create_tool_action(
                    tool_name, error_result, execution_time, tool_call_id
                )
    ...
    async def _execute_tool(
        ...
        if tool_name in self.legacy_tools:
            result = await self._execute_legacy_tool(
                tool_name, sql_query, table_names, catalog, database, schema
            )
        elif tool_name in self.enhanced_tools_set:
            result = await self._execute_enhanced_tool(
                tool_name, sql_query, table_names, catalog, database, schema
            )
        else:
        ...
        if (self.plan_hooks and hasattr(self.plan_hooks, 'query_cache') and
            result and (hasattr(result, 'success') and result.success) or
    ...
    async def _execute_legacy_tool(
        ...
        if tool_name == "describe_table":
            if table_names:
                ...
                return PreflightToolResult(
                    tool_name=tool_name,
                    success=False,
                    error="No table name available"
                )
    ...
    async def _send_tool_error_event(
        self,
        tool_name: str,
        error: str,
        execution_id: str,
