...
class DatusAPIService:
    ...
    async def _run_streaming_workflow_task(
        ...
        try:
            ...
            # Check if it was cancelled by API call (use meta flag to avoid race)
    ...
    async def _generate_registered_stream(self, task_id: str):
        ...
        try:
            ...
            if running_task.meta.get("cancelled"):
                ...
                yield f"event: cancelled\ndata: {to_str({'cancelled': True, 'message': 'Task was cancelled'})}\n\n"
                return
    ...
    async def cancel_all_running_tasks(self, wait_timeout: float = 2.0):
        ...
        logger.info(f"Cancelling {len(tasks_to_cancel)} running tasks during shutdown (timeout={wait_timeout}s)")
        ...
        try:
            ...
            logger.warning(f"Some tasks did not finish within {wait_timeout}s timeout during shutdown")
    ...
    async def cancel_task(task_id: str, current_client: str = _depends_get_current_client):
        ...
        try:
            ...
            if running_task:
                ...
                # NOTE: Don't remove from registry immediately - let the generator detect
                # the cancellation and handle cleanup. The generator will call unregister
                # after finishing its cancellation handling.
